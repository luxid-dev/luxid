...
async fn main() -> luxid::Result<()> {
    ...
    // `cargo run` serves; `cargo run -- routes`, `-- migrate`, `-- openapi`
...
"#
    ...
    // Shipped commented out on purpose. A faster linker is the biggest single
    // lever on rebuild time, but enabling it by default produces a project that
    // does not compile on any machine without mold — and this file gets
    // committed, so it would break teammates too.
    plan.file(
        ".cargo/config.toml",
        r#"# A faster linker is the single biggest lever on rebuild time.
#
...
cargo run -- routes       # the routing table
cargo run -- migrate      # apply migrations
cargo run -- openapi      # the OpenAPI 3.1 document
```
